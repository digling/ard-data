            ...
            for i, l in enumerate(lines):
                ...
                if r:
                    for recs in r:
                        if len(recs) != 3:
                            recs = recs + ['???', '???', '???']
                        D[idx] = [poem, name, str(poem)+'.'+str(stanza), i+1, l, s, recs[0], recs[1],
                                recs[2], c, problems]
                        idx += 1
                else:
                    D[idx] = [poem, name, str(poem)+'.'+str(stanza), i+1, l, s, '', '', '', c,
                            problems]
                    idx += 1
    else:
        ecount += 1
...
with open('../rhymes.tsv', 'w') as f:
    f.write('ID\tPOEM_NUMBER\tPOEM_TITLE\tSTANZA\tLINE_NUMBER\tLINE_IN_SOURCE\tLINE\tRHYMEWORD\tRHYME\tRECONSTRUCTION\tCOMMENT\tPROBLEMS\n')
    for k in sorted(D):
